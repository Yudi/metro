...
export function buildAggregatedRailStatusMessage(
  ...
  const eligible = entries
    .map((entry) => ({ entry, eligibility: notificationEligibility(trigger, now, entry.snapshot.important) }))
    .filter((item): item is { entry: NotificationRailStatusEntry; eligibility: NonNullable<ReturnType<typeof notificationEligibility>> } =>
      item.eligibility !== null && !(trigger.statusMode === 'abnormal' && item.entry.snapshot.normal),
    );
  ...
  const body = hasIssue
    ? [...issueGroups.entries()]
      ...
      .map(([status, count]) => `${count} ${count === 1 ? 'linha' : 'linhas'} com '${status}'`)
      .join('\n')
    : [...normalGroups.entries()]
      .map(([status, lineLabels]) => `${lineLabels.join(', ')}${status ? `: ${status}` : ''}`)
      .join('\n');
  ...
  const expiresAt = new Date(Math.min(
    ...eligible.map(({ eligibility }) => Math.min(
    ...
    payload: {
      notification: {
        ...
        data: {
          ...
          important,
          onActionClick: {
            default: { operation: 'navigateLastFocusedOrOpen', url: first.entry.snapshot.url.replace(/^\/+/, '') || '.' },
            settings: { operation: 'navigateLastFocusedOrOpen', url: 'notifications' },
          },
        },
        actions: [{ action: 'settings', title: 'Gerenciar avisos' }],
