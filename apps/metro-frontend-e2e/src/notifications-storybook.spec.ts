...
const captures = resolve(
  __dirname,
  '../../../.impeccable/review/notifications',
);
...
test('shows cloud triggers on desktop and mobile', async ({ page }) => {
  ...
  await page.goto(
    '/iframe.html?id=pages-notifications--aviso-configurado&viewMode=story',
  );
  await expect(
    page.getByRole('heading', { name: 'Notificações', exact: true }),
  ).toBeVisible();
  await expect(
    page.getByText('Minha ida para a faculdade', { exact: true }),
  ).toBeVisible();
  ...
  await expect(
    targetIdentity.getByText('Esmeralda', { exact: true }),
  ).toBeVisible();
  ...
  await page.screenshot({
    animations: 'disabled',
    path: resolve(captures, 'desktop.png'),
    fullPage: true,
  });
  ...
  await page.screenshot({
    animations: 'disabled',
    path: resolve(captures, 'mobile.png'),
    fullPage: true,
  });
  await page.emulateMedia({ colorScheme: 'dark' });
  await page.screenshot({
    animations: 'disabled',
    path: resolve(captures, 'mobile-dark.png'),
    fullPage: true,
  });
});

test('hides selected lines from general results and reveals an exact search', async ({
  page,
}) => {
  await page.goto(
    '/iframe.html?id=pages-notifications--aviso-configurado&viewMode=story',
  ...
  await page
    .getByRole('button', { name: 'Editar Minha ida para a faculdade' })
    .click();
  const results = page.getByRole('listbox', { name: 'Resultados da busca' });
  await expect(
    results.getByRole('button', { name: 'Linha 9 - Esmeralda', exact: true }),
  ).toHaveCount(0);
...
test('edits multiple days and time ranges without losing existing selections', async ({
  page,
}) => {
  await mkdir(captures, { recursive: true });
  await page.setViewportSize({ width: 1440, height: 1000 });
  ...
  await page
    .getByRole('button', { name: 'Editar Minha ida para a faculdade' })
    .click();
  await expect(page.getByLabel('Nome do aviso')).toHaveValue(
    'Minha ida para a faculdade',
  );
  await page.getByRole('button', { name: 'sábado', exact: true }).click();
  await expect(
    page.getByRole('button', { name: 'sábado', exact: true }),
  ).toHaveAttribute('aria-pressed', 'true');
  await expect(
    page.getByRole('button', { name: 'segunda-feira', exact: true }),
  ).toHaveAttribute('aria-pressed', 'true');
  ...
  await page.screenshot({
    animations: 'disabled',
    path: resolve(captures, 'editor-desktop.png'),
    fullPage: true,
  });
  await page.setViewportSize({ width: 390, height: 844 });
  await page.screenshot({
    animations: 'disabled',
    path: resolve(captures, 'editor-mobile.png'),
    fullPage: true,
  });
  expect(
    await page.evaluate(
...
test('saves, pauses and removes a cloud-backed trigger through the UI', async ({
  page,
}) => {
  await page.goto(
    '/iframe.html?id=pages-notifications--aviso-configurado&viewMode=story',
  ...
  await expect(
    page.getByRole('switch', { name: 'Ativar Ida atualizada' }),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Excluir Ida atualizada' }).click();
  await expect(page.getByText('Ida atualizada', { exact: true })).toHaveCount(
    0,
  );
});

test('shows arrival lead choices and hides irrelevant smart and throttle controls', async ({
  page,
}) => {
  await page.goto(
    '/iframe.html?id=pages-notifications--aviso-configurado&viewMode=story',
  ...
  await page
    .getByRole('option', { name: 'Próximos trens', exact: true })
    .click();
  ...
  await expect(page.getByText('Intervalo mínimo', { exact: true })).toHaveCount(
    0,
  );
  await expect(
    page.getByRole('switch', { name: 'Ativar aviso antecipado' }),
  ).toHaveCount(0);
  await page.setViewportSize({ width: 390, height: 844 });
  await page.screenshot({
    animations: 'disabled',
    path: resolve(captures, 'arrivals-mobile.png'),
    fullPage: true,
  });
  expect(
    await page.evaluate(
...
test('explains sign-in and permission-denied states', async ({ page }) => {
  await page.goto(
    '/iframe.html?id=pages-notifications--sem-login&viewMode=story',
  );
  await expect(
    page.getByRole('heading', { name: 'Entre para configurar avisos' }),
  ).toBeVisible();
  await page.goto(
    '/iframe.html?id=pages-notifications--permissao-bloqueada&viewMode=story',
  );
  await expect(page.getByText(/bloquead/i).first()).toBeVisible();
});

test('identifies an authorized device by browser and operating system', async ({
  page,
}) => {
  await page.goto(
...
test('previews three fixed rail lines in each name format on desktop and mobile', async ({
  page,
}) => {
  await mkdir(captures, { recursive: true });
  await page.setViewportSize({ width: 1440, height: 1000 });
  await page.goto(
    '/iframe.html?id=notifications-trigger-editor--status-de-varias-linhas&viewMode=story',
  );
  const preview = page.locator('.notification-preview');
  for (const [option, first, second, third] of [
    [
      'Nome completo: Linha 1 - Azul',
      'Linha 1 - Azul',
      'Linha 2 - Verde',
      'Linha 3 - Vermelha',
    ],
    ...
    await page
      .getByRole('combobox', { name: 'Nome das linhas no aviso', exact: true })
      .click();
    await page.getByRole('option', { name: option, exact: true }).click();
    await expect(page.locator('.cdk-overlay-pane')).toHaveCount(0);
    await expect(preview.locator('p')).toHaveText(
      `${first}: Velocidade Reduzida\n${second}: Operação normalizada\n${third}: Operação Normal`,
    );
  }
  await expect(
    page.getByRole('combobox', { name: 'Cenário de exemplo', exact: true }),
  ).toHaveCount(0);
  await page
    .locator('.preview-section')
    .screenshot({ path: resolve(captures, 'status-preview-desktop.png') });
  await page.setViewportSize({ width: 390, height: 844 });
  await page
    .getByRole('combobox', { name: 'Nome das linhas no aviso', exact: true })
    .click();
  await page
    .getByRole('option', { name: 'Nome completo: Linha 1 - Azul', exact: true })
    .click();
  await expect(page.locator('.cdk-overlay-pane')).toHaveCount(0);
  expect(
    await page.evaluate(
  ...
  await page
    .locator('.preview-section')
    .screenshot({ path: resolve(captures, 'status-preview-mobile.png') });
  await page.emulateMedia({ colorScheme: 'dark' });
  await page
    .locator('.preview-section')
    .screenshot({ path: resolve(captures, 'status-preview-mobile-dark.png') });
});

test('keeps the line-name preference after saving, pausing and reopening the editor', async ({
  page,
}) => {
  await page.goto(
    '/iframe.html?id=pages-notifications--aviso-configurado&viewMode=story',
  ...
  await page.getByRole('button', { name: 'Salvar aviso', exact: true }).click();
  await page
    .getByRole('switch', { name: 'Desativar Minha ida para a faculdade' })
    .click();
  await page
    .getByRole('button', { name: 'Editar Minha ida para a faculdade' })
    .click();
  await expect(
    page.getByRole('combobox', {
      name: 'Nome das linhas no aviso',
      exact: true,
    }),
  ).toContainText('Cor: Azul');
  await expect(page.locator('.notification-preview p')).toHaveText(
    'Azul: Velocidade Reduzida\nVerde: Operação normalizada\nVermelha: Operação Normal',
  );
});
