...
export function extractBusPlatform(
  ...
  return /\b(?:plataforma|plat)\.?\s*([a-z]|\d{1,3})\b/
    .exec(text)?.[1]
    ?.toUpperCase();
}
...
function compatible(candidate: PhysicalStopCandidate): boolean {
  const left = stopEvidence(
    candidate.sptransName,
    candidate.sptransDescription,
    candidate.sptransPlatform,
  );
  const right = stopEvidence(
    candidate.artespName,
    candidate.artespDescription,
    candidate.artespPlatform,
  );
...
export function matchPhysicalStops(
  ...
  const uniqueNearest = (items: PhysicalStopCandidate[]) => {
    ...
    return items.length > 1 &&
      items[1].distanceMeters - items[0].distanceMeters < 0.5
      ? undefined
      : items[0];
  };
  const leftNearest = new Map(
    [...bySptrans].map(([id, items]) => [id, uniqueNearest(items)]),
  );
  const rightNearest = new Map(
    [...byArtesp].map(([id, items]) => [id, uniqueNearest(items)]),
  );
  return bounded
    .filter(
      (candidate) =>
        leftNearest.get(candidate.sptransStopId) === candidate &&
        rightNearest.get(candidate.artespStopId) === candidate &&
