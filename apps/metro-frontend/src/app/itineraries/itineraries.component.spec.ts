...
const itinerary = (sourceAgency = 'sptrans'): RouteItinerary => ({
  status: 'AVAILABLE',
  serviceDate: saoPauloServiceDate(),
  operatorName: 'Operadora de exemplo',
  route: {
    routeId: 'example',
    shortName: '477A-10',
...
describe('ItinerariesComponent', () => {
  const params = new BehaviorSubject(
    convertToParamMap({ agency: 'sptrans', line: 'example' }),
  );
  const load = jest.fn();
      ...
      providers: [
        ...
        { provide: ItinerariesService, useValue: { load, published } },
        {
          ...
          useValue: {
            search: jest
              .fn()
              .mockReturnValue(of({ success: true, results: [] })),
          },
        },
  ...
  it('shows the plain Sunday gratuity label only for SPTrans', async () => {
    published.mockReturnValue(
      of({ status: 'AVAILABLE', days: [{ kind: 'sunday', directions: [] }] }),
    );
  ...
  it('shows the published timetable and street itinerary without mixing GTFS departures into it', async () => {
    published.mockReturnValue(
      of({
        ...
        routeCode: '477A-10',
        lastUpdated: null,
        operatorName: 'Empresa publicada',
        ...
        days: [
          {
            ...
            directions: [
              {
                id: 'outbound',
                headsign: 'Destino publicado',
                startTime: '04:00',
                endTime: '25:00',
                departures: ['04:20', '25:00'],
                streets: [
                  {
                    name: 'Via publicada',
                    number: '10–20',
                    notices: ['Aviso desta via'],
                  },
                  { name: 'Outra via', number: '' },
                ],
    ...
    expect(fixture.nativeElement.querySelectorAll('.has-notice')).toHaveLength(
      1,
    );
    expect(
      fixture.nativeElement.querySelector('.has-notice').textContent,
    ).toContain('Aviso desta via');
    expect(fixture.nativeElement.querySelector('#exact-departures')).toBeNull();
    fixture.nativeElement
      .querySelector('button[aria-controls="exact-departures"]')
      .click();
    fixture.detectChanges();
    expect(
  ...
  it('cancels a previous route request when navigation selects another line', async () => {
    ...
    load
      ...
      .mockReturnValueOnce(of(itinerary('artesp')));
    const fixture = TestBed.createComponent(ItinerariesComponent);
    fixture.detectChanges();
    await fixture.whenStable();
  ...
  it('does not mistake an unavailable response for an empty service day', async () => {
    load.mockReturnValue(
      of({ ...itinerary(), status: 'UNAVAILABLE', patterns: [] }),
    );
    const fixture = TestBed.createComponent(ItinerariesComponent);
    fixture.detectChanges();
    await fixture.whenStable();
