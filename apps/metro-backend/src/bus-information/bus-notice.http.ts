...
export const BUS_NOTICE_USER_AGENT = 'Projeto-Transporte-Metropolitano-Backend/1.0';
...
export class BusNoticeHttpClient {
  async get(path: string, runSignal: AbortSignal): Promise<string> {
    ...
    if (url.origin !== NOTICE_ORIGIN || url.username || url.password) throw new Error('Invalid notice source');
    const response = await fetch(url, {
      redirect: 'error',
      signal: AbortSignal.any([runSignal, AbortSignal.timeout(20_000)]),
      headers: { 'User-Agent': BUS_NOTICE_USER_AGENT, Accept: 'text/html', 'Accept-Language': 'pt-BR' },
    });
    ...
    try {
      while (true) {
        ...
        if (size > MAX_BYTES) throw new Error('Notice response exceeds size limit');
        chunks.push(value);
      }
    } finally {
      await reader.cancel();
