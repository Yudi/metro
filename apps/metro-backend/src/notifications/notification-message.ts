...
export type NotificationRailStatusTarget = Pick<NotificationRailLine, 'targetId' | 'label' | 'lineCode'>;
...
export function buildNotificationMessage(
  ...
  if (trigger.kind === 'rail_status') {
    return buildAggregatedRailStatusMessage(trigger, triggerId, [{ targetId, snapshot }], now);
  }
  const eligibility = notificationEligibility(trigger, now, snapshot.important);
  if (!eligibility) return null;
  let body = snapshot.body;
  const lineName = formatNotificationLineName(snapshot.lineCode, snapshot.title, trigger.lineNameFormat);
  const title = snapshot.stationName && snapshot.lineCode
  ...
  if (trigger.kind === 'rail_arrivals' || trigger.kind === 'bus_arrivals') {
    ...
    ).sort((left, right) => Number(right.atPlatform === true) - Number(left.atPlatform === true) || left.expectedAt - right.expectedAt);
...
export function readDeliveredRailStates(payload: unknown): Map<string, NotificationRailState> {
  const states = new Map<string, NotificationRailState>();
  if (!payload || typeof payload !== 'object') return states;
  const notification = (payload as { notification?: { data?: { railStates?: unknown } } }).notification;
  const rows = notification?.data?.railStates;
  if (!Array.isArray(rows)) return states;
  for (const row of rows) {
    if (row && typeof row === 'object' && typeof row.targetId === 'string' &&
...
export function railNotificationStateScope(targetIds: readonly string[]): string {
  return notificationHash(JSON.stringify([...new Set(targetIds)].sort()));
}

export function railNotificationLine(entry: NotificationRailStatusEntry): NotificationRailLine {
  return {
    ...entry.snapshot,
...
export function buildAggregatedRailStatusMessage(
  ...
  selectedTargets: readonly NotificationRailStatusTarget[] = entries.map(railNotificationLine),
) {
  if (trigger.kind !== 'rail_status' || !entries.length) return null;
  const eligible = entries.flatMap((entry) => {
    const eligibility = notificationEligibility(trigger, now, entry.snapshot.important);
    return eligibility ? [{ entry, eligibility }] : [];
  }).sort((left, right) => compareNotificationRailLines(railNotificationLine(left.entry), railNotificationLine(right.entry)));
  if (!eligible.length) return null;

  const lines = eligible.map(({ entry }) => railNotificationLine(entry));
  const missingIds = trigger.targetIds.filter((id) => !entries.some((entry) => entry.targetId === id));
  const targetsById = new Map(selectedTargets.map((target) => [target.targetId, target]));
  const missingLines: NotificationRailLine[] = [];
  for (const targetId of missingIds) {
    const target = targetsById.get(targetId);
    if (!target?.label?.trim()) return null;
    missingLines.push({ ...target, normal: false, statusCode: 'DadosIndisponiveis' });
  }
  const recoveredTargetIds = lines.filter((line) => notificationRailState(line) === 'operational' && previousStates.get(line.targetId) === 'issue').map((line) => line.targetId);
  const reopenedTargetIds = lines.filter((line) => notificationRailState(line) === 'operational' && previousStates.get(line.targetId) === 'closed').map((line) => line.targetId);
  const hasAlert = lines.some((line) => ['issue', 'closed'].includes(notificationRailState(line)));
  ...
  if (!hasAlert && !hasRecovery && (missingLines.length || trigger.statusMode === 'abnormal')) return null;
  ...
  const summary = buildRailNotificationSummary([...lines, ...missingLines], {
    ...
    networkAllOperational: eligible.every(({ entry }) => entry.snapshot.networkAllOperational === true),
    recoveredTargetIds,
    reopenedTargetIds,
  });
  const fingerprintEntries = eligible.map(({ entry }) => [
    entry.targetId,
  ...
  const fingerprint = notificationHash(JSON.stringify([first.eligibility.windowKey, fingerprintEntries]));
  const expiresAt = new Date(Math.min(...eligible.map(({ eligibility, entry }) => Math.min(
    Math.floor(now.getTime() / 60_000) * 60_000 + eligibility.remainingMinutes * 60_000,
    now.getTime() + 300_000,
    ...
    payload: {
      notification: {
        ...
        data: {
          ...
          recoveredTargetIds,
          reopenedTargetIds,
          ...railStatusState(trigger.targetIds, eligible.map(({ entry }) => entry), first.eligibility.windowKey, previousStates),
          onActionClick: {
            default: {
              operation: 'navigateLastFocusedOrOpen',
...
function railStatusState(targetIds: readonly string[], entries: readonly NotificationRailStatusEntry[], windowKey: string, previousStates: ReadonlyMap<string, NotificationRailState>) {
  ...
  const remembered = new Map([...previousStates].filter(([targetId]) => targetIds.includes(targetId)));
  for (const { targetId, snapshot } of entries) remembered.set(targetId, notificationRailState(snapshot));
  return {
    stateScope: railNotificationStateScope(targetIds),
    stateFingerprint: notificationHash(JSON.stringify(state)),
    railStates: [...remembered].map(([targetId, state]) => ({ targetId, state })),
    windowKey,
  };
}
