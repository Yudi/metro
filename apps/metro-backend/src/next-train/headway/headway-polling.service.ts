...
import {
  ...
  HEADWAY_POLL_INTERVALS,
  HEADWAY_POLL_JITTER_RATIO,
  MIN_POLL_TICK_INTERVAL,
  WARMUP_MAX_STATIONS,
  WARMUP_OFF_HOURS_MAX_STATIONS,
  STATION_STAGGER_MS,
  OFF_HOURS_STATUS_RECHECK_INTERVAL,
  NORMAL_HOURS_POLL_INTERVAL,
  NON_OPERATING_STATUS_CODES,
  PRUNE_INTERVAL,
  HeadwayProvider,
  getJitteredInterval,
  getStartupJitterOffset,
  shuffleCptmPollingStations,
  shuffleStations,
} from './headway-polling.config';
...
export class HeadwayPollingService implements OnModuleInit, OnModuleDestroy {
  ...
  private async startLinePolling(
    ...
    const shuffledStations = shuffleStations(stationCodes);
    ...
    for (let i = 0; i < warmupCount; i++) {
      ...
      const warmupDelay =
        i * STATION_STAGGER_MS + getStartupJitterOffset(STATION_STAGGER_MS);
  ...
  private async registerCptmLinePolling(
    ...
    for (const stationCode of shuffleStations(stationCodes)) {
    ...
    shuffleCptmPollingStations(this.cptmPollingStations);
  ...
  private scheduleNextLinePoll(
    ...
    const delay = getJitteredInterval(baseTickInterval);
    const timer = setTimeout(async () => {
      if (!this.isRunning || !this.activePollers.has(timerKey)) return;
