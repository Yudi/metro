...
export function assertNoticeHtml(html: string): void {
  if (/cf-chl-running|cf-chl-widget|px-captcha|<title>\s*(?:Just a moment|Access Denied)/i.test(html)) {
...
export function noticeUrl(href: string): { sourceId: string; sourceUrl: string } | null {
  try {
    const url = new URL(href, NOTICE_ORIGIN);
    const match = /^\/informativos\/[a-z-]+\/[a-z0-9-]+\/(\d+)\/?$/.exec(url.pathname);
    if (url.origin !== NOTICE_ORIGIN || url.username || url.password || url.search || url.hash || !match) return null;
...
export function parseNoticeIndex(html: string): NoticeReference[] {
  ...
  for (const group of groups) {
    ...
    const heading = $('h2').filter((_, element) => $(element).text().trim() === group.heading);
    const container = heading.parent();
    if (heading.length !== 1 || container.find('ul.alteracoesOP').length !== 1) {
    ...
    container.find('ul.alteracoesOP li').each((_, element) => {
      ...
      if (!identity || !title || !listedDate) throw new Error('Incomplete notice index entry');
  ...
  if (!result.length || result.length > 1000) throw new Error('Unexpected notice index size');
  return result;
}

export function parseNoticeDetail(html: string, reference: NoticeReference): BusNotice {
  assertNoticeHtml(html);
  const $ = load(html);
  ...
  const description = body.text().split('\n').map((line) => line.replace(/\s+/g, ' ').trim())
    .filter(Boolean).join('\n');
  if (!title || title.length > 500 || description.length < 25 || description.length > 100_000) {
    throw new Error('Unrecognized notice detail structure');
  }
  const routes = [...new Set(`${title}\n${description}`.toUpperCase().match(/\b[0-9A-Z]{4}-\d{2}\b/g) ?? [])];
  // Retain the source wording. A publication date or "recent" bucket is not an active period.
  return { ...reference, title, description, routes, periodText: description.split('\n')[0] };
}
