...
describe('NotificationSnapshotService', () => {
  ...
  it('reads fixed special-line departures and ignores a special line with no departure', async () => {
    ...
    await expect(
      ...
      title: 'Próximas partidas - Expresso Aeroporto',
      normal: true,
    });
    await expect(
      setup.service.read(
  ...
  it.each(['OperacaoNormal', 'OperacaoTransitoria'] as const)(
    'allows a complete network all-clear with %s',
    async (statusCode) => {
      const setup = service();
      setup.rail.getLinesStatus.mockResolvedValue({
        success: true,
        lastUpdated: NOW,
        lines: RAIL_LINES.map((line) => ({
          code: line.code,
          line: line.fullName,
          statusCode,
          statusLabel: STATUS_CODE_TO_LABEL[statusCode],
        })),
      });
      const snapshot = await setup.service.read(
        'rail_status',
  ...
  it.each([
    ...
    'keeps the all-clear scoped when an unselected line is %s',
    async (state) => {
      const setup = service();
      const lines = RAIL_LINES.map((line) => ({
        ...
        line: line.fullName,
        statusCode:
          line.code === 2 && state !== 'missing' ? state : 'OperacaoNormal',
        statusLabel: 'Operação Normal',
      }));
      setup.rail.getLinesStatus.mockResolvedValue({
        success: true,
        lastUpdated: NOW,
        lines:
          state === 'missing' ? lines.filter((line) => line.code !== 2) : lines,
      });
      const snapshot = await setup.service.read(
        'rail_status',
        target('rail_line', { lineCode: 'L1' }),
  ...
  it('does not keep a cached all-clear beyond the observation freshness limit', async () => {
    ...
    setup.rail.getLinesStatus.mockResolvedValue({
      success: true,
      lastUpdated: new Date(NOW.getTime() - 599_000),
      lines: [
        {
          code: 1,
          line: 'Linha 1 - Azul',
    ...
    expect(
      await setup.service.read('rail_status', selection, NOW),
    ).not.toBeNull();
    expect(
      await setup.service.read(
        'rail_status',
  ...
  it.each(['OperacaoEncerrada', 'Paralisada'])(
    'suppresses special departures when the service is %s',
    async (statusCode) => {
      const setup = service();
      setup.specialRail.getSpecialLinesStatus.mockResolvedValue([
        {
          ...
          line: 'Expresso Aeroporto',
          statusCode,
          nextDepartures: [{ label: 'Aeroporto', time: '09:30' }],
          issues: [],
        },
      ]);
      expect(
