...
const notice: OperationalNotice = {
  ...
  routes: ['875A-10', '975A-10', '1234-10'],
  listing: 'RECENT',
...
describe('route warning presentation', () => {
  it('extracts shared-route direction blocks without leaking the next route instructions', () => {
    for (const code of ['875A-10', '975A-10']) {
      ...
      expect(view.directions).toEqual([
        { label: 'Ida', text: 'via alternativa.' },
        { label: 'Volta', text: 'sem alteração.' },
      ]);
      expect(JSON.stringify(view)).not.toContain('rua exclusiva');
    }
    expect(routeNoticeView(notice, '1234-10').directions).toEqual([
      { label: 'Ida', text: 'rua exclusiva de outra linha.' },
    ]);
  });
  it('stops at an unrecognized route heading instead of attributing its directions to the previous route', () => {
    const changed = {
      ...notice,
      description: notice.description.replace(
        '1234-10 Outro destino',
        'Linha 1234-10 Outro destino',
      ),
    };
    expect(JSON.stringify(routeNoticeView(changed, '875A-10'))).not.toContain(
      'rua exclusiva',
    );
  ...
  it('fails safely for ambiguous route sections, periods and unsafe source links', () => {
    const view = routeNoticeView(
      {
        ...notice,
        periodText: 'Informativo',
  ...
  it('starts with a route-specific button, expands parsed fields, and has no global list or intervals', async () => {
    await TestBed.configureTestingModule({
      imports: [BusInformationComponent],
    }).compileComponents();
    ...
    fixture.componentRef.setInput('notices', [
      routeNoticeView(notice, '875A-10'),
    ]);
