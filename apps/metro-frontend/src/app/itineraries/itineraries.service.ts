...
export interface ItineraryPattern {
  ...
  headsign: string;
  stops: { id: string; name: string; sequence: number; latitude: number; longitude: number }[];
  departures: string[];
  intervals: { startTime: string; endTime: string; headwaySeconds: number; exactTimes: boolean }[];
...
export class ItinerariesService {
  ...
  published(routeId: string) {
    return this.http.post<{
      ...
      timeout(20_000),
      map((response) => {
        if (response.errors?.length || !response.data?.busPublishedRouteInformation) {
