...
export function routeNoticeView(
  notice: OperationalNotice,
  routeCode: string,
): RouteNoticeView {
  const lines = notice.description
    .split('\n')
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  for (const line of lines) {
    ...
    if (route) {
      ...
      group.routes.push(
        ...(line.toUpperCase().match(/\b[0-9A-Z]{4}-\d{2}\b/g) ?? []),
      );
  ...
  const firstRouteLine = lines.findIndex((line) =>
    /\b[0-9A-Z]{4}-\d{2}\b/i.test(line),
  );
  const introduction =
    firstRouteLine < 0 ? lines : lines.slice(0, firstRouteLine);
  const reason =
    introduction
      .find((line) => /^Motivo\s*:/i.test(line))
      ?.replace(/^Motivo\s*:\s*/i, '') ?? null;
  return {
    id: notice.sourceId,
    title: notice.title,
    period:
      /\d{1,2}\/\d{1,2}|\b(?:das|dia|dias|até|entre|partir|feira|sábado|domingo)\b/i.test(
        period,
      )
        ? period
        : null,
    ...
    sourceUrl:
      /^https:\/\/www\.sptrans\.com\.br\/informativos\/[a-z-]+\/[a-z0-9-]+\/\d+\/?$/.test(
        notice.sourceUrl,
      )
        ? notice.sourceUrl
        : null,
