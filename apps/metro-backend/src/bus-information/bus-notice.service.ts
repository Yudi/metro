...
const INDEX_PATHS = [
  ...Array.from({ length: 8 }, (_, i) => `/informativos/regiao/${i + 1}`),
  '/informativos/varias-areas',
];
  ...
  implements OnApplicationBootstrap, OnModuleDestroy
{
  ...
  constructor(
    ...
    private readonly http: BusNoticeHttpClient,
  ) {}
  ...
  async refresh(now = new Date()): Promise<void> {
    ...
    try {
      ...
      for (const path of INDEX_PATHS) {
        ...
        for (const reference of parseNoticeIndex(
          await this.http.get(path, signal),
        )) {
          const previous = references.get(reference.sourceId);
          if (!previous || reference.listing === 'UPCOMING')
            references.set(reference.sourceId, reference);
        }
      }
      if (references.size > 1000)
        throw new Error('Unexpected aggregate notice count');
      const cache = Object.fromEntries(
        Object.entries(state.detailCache).filter(([id]) => references.has(id)),
      );
      ...
      const pending = [...references.values()]
        .filter((reference) => {
          ...
          return (
            !saved ||
            saved.notice.sourceUrl !== reference.sourceUrl ||
            saved.notice.title !== reference.title ||
            saved.notice.listedDate !== reference.listedDate ||
            now.getTime() - Date.parse(saved.fetchedAt) >= REFRESH_MS
          );
        })
        .sort((a, b) =>
          (cache[a.sourceId]?.fetchedAt ?? '').localeCompare(
            cache[b.sourceId]?.fetchedAt ?? '',
          ),
        );
      for (const reference of pending.slice(0, 120)) {
        await delay(2000, undefined, { signal });
        const notice = parseNoticeDetail(
          await this.http.get(reference.sourceUrl, signal),
          reference,
        );
      ...
      if (pending.length > 120)
        throw new Error('Notice detail budget reached; continuing next window');
      const notices = [...references.values()].map((reference) => ({
        ...cache[reference.sourceId].notice,
        ...reference,
      }));
      const retainedCache = Object.fromEntries(
        [...references.keys()].map((id) => [id, cache[id]]),
      );
      await this.prisma.$executeRaw`
      ...
      this.logger.warn(
        `Notice refresh skipped or failed; keeping last complete snapshot: ${error instanceof Error ? error.message : 'unknown error'}`,
      );
    } finally {
      this.running = false;
    }
