import {
  ScheduledBusService,
  ScheduledStopTimeRow,
  selectScheduledDepartures,
} from './scheduled-bus.service';

const row = (
  overrides: Partial<ScheduledStopTimeRow> = {},
): ScheduledStopTimeRow => ({
  routeId: 'artesp:001',
  routeShortName: '001',
  tripId: 'artesp:trip-1',
  headsign: 'Metrô Capão Redondo',
  directionId: 0,
  departure: '12:00:00',
  stopName: 'Terminal',
  startDate: '20260101',
...
describe('published Artesp departures', () => {
  ...
  it('keeps Friday service at 26:59 on Saturday after its calendar end date', () => {
    ...
    expect(departures.map((departure) => departure.departureTime)).toEqual([
      ...
      selectScheduledDepartures(
        [
          row({
            ...
            endDate: null,
            weekdays: [],
            exceptions: [{ date: '20260905', exceptionType: 1 }],
          }),
        ],
        now,
        12,
      ).map((departure) => departure.departureTime),
    ).toEqual(['2026-09-05T15:00:00.000Z']);
    expect(
      selectScheduledDepartures(
        [
          row({
            startDate: '20260905',
            endDate: '20260905',
            exceptions: [{ date: '20260905', exceptionType: 2 }],
          }),
        ],
        now,
        12,
  ...
  it('never returns past departures, malformed times, or expired service', () => {
    expect(
      selectScheduledDepartures([row()], new Date('2027-01-07T13:00:00Z'), 12),
    ).toEqual([]);
    expect(
      selectScheduledDepartures(
        [row({ departure: '27:61:00' })],
        new Date('2026-09-05T13:00:00Z'),
        12,
      ),
    ).toEqual([]);
    const departures = selectScheduledDepartures(
      [row({ endDate: '20260905' })],
  ...
  it('covers the importer maximum 99-hour GTFS time and labels the feed', () => {
    ...
    expect(departures[0]).toMatchObject({
      departureTime: '2026-09-05T06:00:00.000Z',
      sourceAgency: 'artesp',
      platformCode: 'A',
    });
  });
  ...
  it('sorts and limits distinct departures even if trip rows arrive unordered', () => {
    ...
    expect(departures.map((departure) => departure.tripId)).toEqual([
      'artesp:trip-1',
      'artesp:later',
    ]);
  });
    ...
    const departures = selectScheduledDepartures(
      [
        ...Array.from({ length: 6 }, (_, index) =>
          row({
            routeId: 'artesp:frequent',
            routeShortName: '010',
            tripId: `artesp:frequent-${index}`,
            departure: `${String(12 + index).padStart(2, '0')}:00:00`,
          }),
        ),
        row({
          routeId: 'artesp:less-frequent',
          routeShortName: '020',
          tripId: 'artesp:less-frequent-1',
          departure: '18:00:00',
        }),
      ],
      now,
  ...
  it('caps each route independently at five departures', () => {
    const departures = selectScheduledDepartures(
      [
        ...
        ...Array.from({ length: 2 }, (_, index) =>
          row({
            routeId: 'artesp:other',
            routeShortName: '020',
            tripId: `artesp:other-${index}`,
    ...
    expect(
      departures.filter((departure) => departure.routeId === 'artesp:frequent'),
    ).toHaveLength(5);
    expect(
      departures.filter((departure) => departure.routeId === 'artesp:other'),
    ).toHaveLength(2);
  });
});
...
describe('ScheduledBusService request scope', () => {
  ...
  it.each([0, -1, 51, 1.5])(
    ...
    async (limit) => {
      const query = jest.fn();
      const service = new ScheduledBusService({ $queryRaw: query } as never);
      await expect(service.getDepartures('42', limit)).rejects.toMatchObject({
        status: 400,
      });
  ...
  it.each([0, -1, 6, 1.5])(
    'rejects invalid per-route limits before querying: %s',
    async (perRouteLimit) => {
      const query = jest.fn();
      const service = new ScheduledBusService({ $queryRaw: query } as never);
      await expect(
        service.getDepartures('42', 12, undefined, perRouteLimit),
