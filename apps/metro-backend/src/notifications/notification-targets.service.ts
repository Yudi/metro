...
export class NotificationTargetsService {
  ...
  async search(kind: NotificationTargetKind, search: string): Promise<NotificationTarget[]> {
    if (!['rail_line', 'rail_station', 'bus_route', 'bus_stop', 'special_line'].includes(kind) || typeof search !== 'string' || search.length > 100) throw new BadRequestException('Busca inválida.');
    const term = normalizeNotificationName(search);
    let candidates: Candidate[];
    if (kind === 'rail_line') {
      candidates = RAIL_LINES.map(line => ({ kind, label: line.fullName, descriptor: { lineCode: line.lineId } }));
    } else if (kind === 'rail_station') {
      candidates = RAIL_LINES.filter(line => HEADWAY_LINE_CODES.has(line.lineId)).flatMap(line => line.stations.map(station => ({ kind, label: `${station.name} · ${line.fullName}`, descriptor: { lineCode: line.lineId, stationCode: station.code } })));
    } else if (kind === 'special_line') {
      candidates = [
        { kind, label: 'Expresso Aeroporto', descriptor: { code: SPECIAL_RAIL_LINE_CODES.EXPRESSO_AEROPORTO } },
        { kind, label: 'Expresso Linha 10', descriptor: { code: SPECIAL_RAIL_LINE_CODES.EXPRESSO_LINHA_10 } },
      ];
      ...
      `;
      candidates = rows.map(row => ({ kind, label: `${row.name} · ${row.latitude.toFixed(5)}, ${row.longitude.toFixed(5)}`, descriptor: { name: row.name, description: row.description ?? '', latitude: row.latitude, longitude: row.longitude, platformCode: row.platformCode ?? '' } }));
    }
    const filtered = kind.startsWith('bus_') ? candidates : candidates.filter(c => normalizeNotificationName(c.label).includes(term));
  ...
  private railLinePresentation(candidate: Candidate): Pick<NotificationTarget, 'railLineCode'> {
