...
const pair = (
  overrides: Partial<PhysicalStopCandidate> = {},
): PhysicalStopCandidate => ({
...
describe('physical bus stop matching', () => {
  ...
  it.each([
    ...
    {
      artespName: 'R. Tibúrcio de Souza, 3200 B/C',
      sptransName: 'R. Tibúrcio de Souza, 3200 C/B',
    },
    ...
    'preserves separate boarding points for conflicting or insufficient evidence: %p',
    (overrides) => {
  ...
  it('matches only mutual nearest neighbours, rejecting near-equal alternatives', () => {
    expect(
      matchPhysicalStops([
        pair(),
        pair({ artespStopId: 'artesp:602', distanceMeters: 0.8 }),
      ]),
    ).toEqual([]);
    expect(
      matchPhysicalStops([
        pair(),
        pair({ sptransStopId: 'other', distanceMeters: 3 }),
      ]),
  ...
  it('recognizes explicit platform metadata in both feeds without inventing it', () => {
    expect(extractBusPlatform('Terminal Barra Funda Lado Norte Plat. A')).toBe(
      'A',
    );
