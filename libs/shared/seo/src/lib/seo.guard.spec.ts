...
describe('city metadata', () => {
  ...
  it('preserves the deployment base and replaces the canonical link on child navigation', () => {
    ...
    guard.canActivateChild(route, {
      url: '/sp/mapa?lat=1#map',
    } as RouterStateSnapshot);
    guard.canActivateChild(route, { url: '/sp/sobre' } as RouterStateSnapshot);
    const links = TestBed.inject(DOCUMENT).head.querySelectorAll(
      'link[rel="canonical"]',
    );
    expect(TestBed.inject(Location).prepareExternalUrl('/sp/sobre')).toBe(
      '/app/sp/sobre',
    );
    expect(links.length).toBe(1);
    expect(links[0].getAttribute('href')).toBe(
      'https://metro.yudi.com.br/app/sp/sobre',
    );
  });
  ...
  it('uses the selected city title and description without duplicating the site title', () => {
    ...
    const city = {
      ...DEFAULT_CITY,
      siteTitle: 'Transporte de outra cidade',
