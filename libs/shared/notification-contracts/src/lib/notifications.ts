...
export const TARGET_KIND_FOR_NOTIFICATION: Record<
  NotificationKind,
  NotificationTargetKind
> = {
  rail_status: 'rail_line',
  rail_headway: 'rail_station',
  rail_arrivals: 'rail_station',
  bus_arrivals: 'bus_stop',
  bus_notices: 'bus_route',
  special_departures: 'special_line',
};
...
export function validateNotificationTrigger(value: unknown): string | null {
  ...
  if (typeof v.name !== 'string' || !v.name.trim() || v.name.trim().length > 80)
    return 'Dê um nome de até 80 caracteres ao aviso.';
  if (!NOTIFICATION_KINDS.includes(v.kind as NotificationKind))
    return 'Selecione o tipo de aviso.';
  if (typeof v.enabled !== 'boolean' || typeof v.smart !== 'boolean')
  ...
  if (
    ...
    v.windows.some(
      (w) =>
        !w || !validTime(w.start) || !validTime(w.end) || w.start === w.end,
    )
  )
    return 'Informe até oito faixas de horário com início e fim diferentes.';
  if (v.timezone !== NOTIFICATION_TIMEZONE)
    return 'Os avisos usam exclusivamente o horário de São Paulo.';
  if (
    !Number.isInteger(v.leadMinutes) ||
    (v.leadMinutes ?? -1) < 0 ||
    (v.leadMinutes ?? 61) > 60
  )
  ...
  if (
    ...
    (v.intervalMinutes ?? 121) > 120
  )
    return `O intervalo deve estar entre ${minimumInterval} e 120 minutos.`;
  if (
    v.arrivalLeadMinutes !== undefined &&
    (!Number.isInteger(v.arrivalLeadMinutes) ||
      v.arrivalLeadMinutes < 1 ||
      v.arrivalLeadMinutes > 30)
  )
    return 'A antecedência da chegada deve estar entre 1 e 30 minutos.';
  if (
