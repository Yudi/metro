...
export function nextNotificationEvaluation(
  ...
  for (let offset = 0; offset <= 7; offset++) {
    ...
    for (const window of trigger.windows) {
      const start = day
        ...
        .subtract({ minutes: lead });
      if (start.epochMilliseconds > now.getTime())
        earliest = Math.min(earliest, start.epochMilliseconds);
    }
  }
