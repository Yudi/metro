...
function serviceDays(now: Date) {
  ...
  const offsets = new Intl.DateTimeFormat('en-US', { timeZone: TIME_ZONE, timeZoneName: 'longOffset' });
  ...
  return Array.from({ length: LOOKAHEAD_DAYS + 5 }, (_, index) => {
    ...
    const offsetName = offsets.formatToParts(new Date(civilDate.getTime() + DAY_MS / 2))
      .find((item) => item.type === 'timeZoneName')?.value ?? '';
    const offset = /GMT([+-])(\d{2}):(\d{2})/.exec(offsetName);
    if (!offset) throw new Error('Unable to determine GTFS service timezone');
    const offsetMs = (Number(offset[2]) * 60 + Number(offset[3])) * 60_000 * (offset[1] === '+' ? 1 : -1);
    return {
...
export function selectScheduledDepartures(
  rows: readonly ScheduledStopTimeRow[], now: Date, limit: number, perRouteLimit?: number,
  ...
  for (const row of rows) {
    ...
    for (const day of days) {
      const exceptions = row.exceptions?.filter((exception) => exception.date === day.date) ?? [];
      if (exceptions.some((exception) => exception.exceptionType === 2)) continue;
      const added = exceptions.some((exception) => exception.exceptionType === 1);
      if (!added && !(row.startDate && row.endDate && day.date >= row.startDate &&
        day.date <= row.endDate && row.weekdays[day.weekday] === 1)) continue;
      ...
      seen.add(key);
      result.push({
        routeId: row.routeId, routeShortName: row.routeShortName, tripId: row.tripId,
        headsign: row.headsign, directionId: row.directionId, departureTime,
        sourceAgency: 'artesp',
        platformCode: extractBusPlatform(row.stopName, row.stopDescription, row.platformCode),
      });
    }
  }
  const sorted = result.sort((a, b) => a.departureTime.localeCompare(b.departureTime) ||
    a.routeShortName.localeCompare(b.routeShortName) || a.tripId.localeCompare(b.tripId));
...
export class ScheduledBusService {
  ...
  async getDepartures(
    stopId: string, limit = 12, now = new Date(), perRouteLimit?: number,
  ): Promise<ScheduledBusDeparture[]> {
    if (!stopId.trim() || stopId.length > 128 || [...stopId].some((character) => character.charCodeAt(0) < 32 || character.charCodeAt(0) === 127)) {
      throw new BadRequestException('Invalid stopId');
    }
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 50) {
      throw new BadRequestException('limit must be between 1 and 50');
    }
    if (perRouteLimit !== undefined &&
      (!Number.isSafeInteger(perRouteLimit) || perRouteLimit < 1 || perRouteLimit > 5)) {
