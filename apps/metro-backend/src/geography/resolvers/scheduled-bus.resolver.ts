...
export class ScheduledBusResolver {
  ...
  @Query(() => [ScheduledBusDeparture], {
    description: 'Upcoming published Artesp departures within seven days, in America/Sao_Paulo service time. These are schedules, not live arrival predictions.',
  })
  scheduledBusDepartures(
    @Args('stopId') stopId: string,
    @Args('limit', { type: () => Int, defaultValue: 12 }) limit: number,
    @Args('perRouteLimit', { type: () => Int, nullable: true }) perRouteLimit?: number | null,
  ): Promise<ScheduledBusDeparture[]> {
    return this.schedules.getDepartures(stopId, limit, undefined, perRouteLimit ?? undefined);
  }
}
