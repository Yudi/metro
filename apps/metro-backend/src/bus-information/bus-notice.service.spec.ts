...
function setup() {
  let claimed = false;
  const state = {
    attemptedWindow: '2026-09-07:0',
    successfulWindow: '2026-09-06:1',
    lastSuccessAt: new Date('2026-09-06T19:30:00Z'),
    notices: [notice],
    detailCache: {},
  };
  const prisma = {
  ...
  const http = {
    get: jest.fn(async (url: string) =>
      url.includes('/71116/') ? detail : index,
    ),
  };
  const create = () =>
    new BusNoticeService(
      prisma as unknown as PrismaService,
      http as BusNoticeHttpClient,
    );
...
describe('twice daily notice collection', () => {
  it.each([
    ['2026-09-07T07:22:00Z', null],
    ['2026-09-07T07:23:00Z', '2026-09-07:0'],
    ['2026-09-07T14:59:00Z', '2026-09-07:0'],
    ['2026-09-07T15:00:00Z', null],
    ['2026-09-07T19:22:00Z', null],
    ['2026-09-07T19:23:00Z', '2026-09-07:1'],
    ['2026-09-08T02:59:00Z', '2026-09-07:1'],
    ['2026-09-08T03:00:00Z', null],
  ])('uses Sao Paulo windows at %s', (date, expected) =>
    expect(noticeWindow(new Date(date))).toBe(expected),
  );
  ...
  it('claims before requests, deduplicates details, and skips warmup across replicas/restarts', async () => {
    ...
    expect(
      prisma.$executeRaw.mock.calls.filter(([sql]) =>
        sql.join('').includes('"successfulWindow" ='),
      ),
    ).toHaveLength(1);
  });
  ...
  it('stops on a block, keeps the previous snapshot and spends the failed window', async () => {
    ...
    expect(await create().forRoutes(['875A-10'], now)).toMatchObject({
      status: 'STALE',
      notices: [notice],
    });
  });
  ...
  it('does not expose unrelated route notices or turn a missing table into a successful empty result', async () => {
    ...
    expect(await create().forRoutes(['875A-10'], now)).toEqual({
      status: 'UNAVAILABLE',
      lastUpdated: null,
      notices: [],
    });
