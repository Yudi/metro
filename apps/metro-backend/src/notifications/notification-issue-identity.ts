...
export async function notificationIssueIdentity(
  tx: Prisma.TransactionClient, kind: NotificationKind, targetId: string, snapshot: NotificationSnapshot,
): Promise<string | null | undefined> {
  if (kind === 'bus_notices') return notificationHash(`bus-notice-${snapshot.fingerprint}`);
  ...
  const target = await tx.notificationTarget.findUniqueOrThrow({ where: { id: targetId } });
  if (target.observationAt && snapshot.observedAt < target.observationAt) return undefined;
  if (target.observationAt?.getTime() === snapshot.observedAt.getTime() && target.observationClass !== observationClass) return undefined;
  let episode = target.observationEpisode;
  if (target.observationClass !== observationClass || !episode) {
    episode = randomUUID();
  }
  await tx.notificationTarget.update({ where: { id: targetId }, data: { observationClass, observationEpisode: episode, observationAt: snapshot.observedAt } });
