...
const dashboardRoute = spFeatureRoutes.find((route) => route.path === 'painel');
...
describe('Dashboard tab retention', () => {
  ...
  it('restores the same panel and DOM without repeating its initial lookup', async () => {
    ...
    const panel = await harness.navigateByUrl(
      '/sp/painel',
    ...
    for (let visit = 0; visit < 3; visit++) {
      ...
      const restored = await harness.navigateByUrl(
        '/sp/painel',
        InsightsDashboardComponent,
      );
  ...
  it('applies favorite changes made in another tab to the retained panel', async () => {
    ...
    http
      .expectOne(
        (request) => request.body.variables?.routeIds?.[0] === 'route-1',
      )
      .flush({
        data: {
          multipleBusRoutes: [
            { routeId: 'route-1', shortName: '1', longName: 'Test route' },
          ],
          multipleBusStops: [],
        },
      });
  ...
  it('keeps the arrival subscription and receives deltas while another tab is open', async () => {
    favorites.set({
      ...emptyFavorites,
      railStation: [getRailStationFavoriteKey('Pinheiros')],
    });
    const harness = await RouterTestingHarness.create();
    await harness.navigateByUrl('/sp/painel', InsightsDashboardComponent);
    const http = TestBed.inject(HttpTestingController);
    ...
    await harness.navigateByUrl('/sp/painel', InsightsDashboardComponent);
    expect(
      socket.emit.mock.calls.filter(([event]) => event === 'subscribe_station'),
    ).toHaveLength(1);
    expect(service.getStationData('L9', 'PIN')?.hasError).toBe(true);
    http.expectNone('/api/graphql');
    ...
    await harness.navigateByUrl('/sp/favoritos', OtherTabComponent);
    (
      TestBed.inject(RouteReuseStrategy) as DashboardRouteReuseStrategy
    ).ngOnDestroy();
