...
describe('notification page realtime consistency', () => {
  ...
  let api: { getConfiguration: jest.Mock; saveTrigger: jest.Mock; registerDevice: jest.Mock };
  let socket: { events$: Subject<NotificationConfigurationRealtimeEvent>; connect: jest.Mock; disconnect: jest.Mock; requestResync: jest.Mock; connected: ReturnType<typeof signal<boolean>> };
  const storageKey = 'metro.notifications.device.realtime-user';
  const trigger: NotificationTrigger = {
    id: 'trigger-one', revision: 2, name: 'Ida', enabled: true,
    days: [1], windows: [{ start: '07:00', end: '09:00' }],
    timezone: 'America/Sao_Paulo', smart: false, leadMinutes: 0,
    intervalMinutes: 15, kind: 'rail_status', targetIds: ['line-one'],
    statusMode: 'all', targets: [{ id: 'line-one', kind: 'rail_line', label: 'Linha 1', available: true, railLineCode: 1 }],
  };
  const configuration: NotificationConfiguration = {
    revision: 10, available: true, publicKey: 'key', triggers: [trigger],
    devices: [{ id: 'device-one', label: 'Chrome · Windows', createdAt: '2026-09-08T12:00:00Z' }],
  };
  ...
  beforeEach(async () => {
    ...
    socket = { events$: events, connect: jest.fn(), disconnect: jest.fn(), requestResync: jest.fn(), connected: signal(true) };
    api = { getConfiguration: jest.fn().mockReturnValue(of(configuration)), saveTrigger: jest.fn(), registerDevice: jest.fn() };
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: NotificationPushService, useValue: {
          permission: signal('granted'), supported: signal(true),
          refreshPermission: jest.fn(), existingSubscription: jest.fn().mockResolvedValue(null),
        } },
      ],
    }).overrideComponent(NotificationsComponent, { set: { template: '' } }).compileComponents();
  ...
  it('applies another device change without another HTTP configuration request', () => {
    delta({ type: 'trigger_upsert', revision: 11, trigger: { ...trigger, revision: 3, name: 'Volta' } });
  ...
  it('resynchronizes a missing delta instead of applying an incomplete sequence', () => {
    delta({ type: 'trigger_upsert', revision: 12, trigger: { ...trigger, revision: 4, name: 'Volta' } });
    expect(socket.requestResync).toHaveBeenCalledTimes(1);
    expect(page.triggers()[0].name).toBe('Ida');
    events.next({ type: 'snapshot', configuration: { ...configuration, revision: 12, triggers: [] } });
  ...
  it('retries synchronization if its snapshot is older than an already observed delta', () => {
    delta({ type: 'trigger_upsert', revision: 13, trigger: { ...trigger, revision: 5, name: 'Volta' } });
    events.next({ type: 'snapshot', configuration: { ...configuration, revision: 12 } });
    expect(socket.requestResync).toHaveBeenCalledTimes(2);
    events.next({ type: 'snapshot', configuration: { ...configuration, revision: 13, triggers: [{ ...trigger, revision: 5, name: 'Volta' }] } });
    expect(page.triggers()[0].name).toBe('Volta');
  });
  ...
  it('keeps the editor open when the same account refreshes its token', async () => {
    ...
    expect(socket.connect).toHaveBeenLastCalledWith('realtime-user', 'refreshed-token');
    expect(api.getConfiguration).toHaveBeenCalledTimes(1);
  });
  ...
  it('does not restore a remotely deleted trigger from a late mutation response', () => {
    ...
    page.saveTrigger({ id: trigger.id, expectedRevision: trigger.revision, input: trigger });
    delta({ type: 'trigger_remove', revision: 11, triggerId: trigger.id, triggerRevision: 3 });
  ...
  it('does not restore a deleted trigger when an earlier commit publishes late', () => {
    delta({ type: 'trigger_remove', revision: 11, triggerId: trigger.id, triggerRevision: 3 });
    delta({ type: 'trigger_upsert', revision: 12, trigger: { ...trigger, revision: 3 } });
    expect(page.triggers()).toEqual([]);
    expect(page.configuration()?.revision).toBe(12);
  });
  ...
  it('clears a removed current device and ignores a late upsert for its deleted ID', () => {
    ...
    delta({ type: 'device_upsert', revision: 12, device: configuration.devices[0] });
  ...
  it('does not close the editor when a stale HTTP snapshot finishes after a delta', () => {
    ...
    delta({ type: 'trigger_upsert', revision: 11, trigger: { ...trigger, revision: 3 } });
  ...
  it('keeps a successful socket snapshot visible if an older HTTP request fails', () => {
    ...
    api.getConfiguration.mockReturnValue(response);
    page.refreshConfiguration();
    events.next({ type: 'snapshot', configuration: { ...configuration, revision: 11 } });
  ...
  it('upgrades the current generic device label once without requesting permission', async () => {
    const existing = jest.mocked(TestBed.inject(NotificationPushService).existingSubscription);
    existing.mockResolvedValue({ endpoint: 'https://push.example/device', label: 'Safari · iOS', keys: { p256dh: 'key', auth: 'key' } });
    api.registerDevice.mockReturnValue(of('device-one'));
    const genericConfiguration = { ...configuration, revision: 11, devices: [{ ...configuration.devices[0], label: 'Dispositivo' }] };
