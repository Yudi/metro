...
jest.mock('./rail-map-context.service', () => ({
  RailMapContextService: class {},
}));
...
describe('CptmVehiclePollingService', () => {
  ...
  it('reuses a completed line snapshot for another client and publishes expiry renewals', async () => {
    const provider = {
      getVehiclesForLine: jest.fn().mockResolvedValue([vehicle]),
    };
    const context = { stations: [], paths: [] };
    const service = new CptmVehiclePollingService(
      provider as never,
      {
    ...
    expect(service.subscribe('second-client', 'L9')?.vehicles).toEqual([
      vehicle,
    ]);
    ...
    provider.getVehiclesForLine.mockResolvedValue([
      { ...vehicle, validUntil: 91_000 },
    ]);
    await service['poll']();
    expect(listener).toHaveBeenLastCalledWith([
      expect.objectContaining({
        vehicles: [{ ...vehicle, validUntil: 91_000 }],
      }),
    ]);
  ...
  it('does not restore a removed line when an in-flight request completes', async () => {
    ...
    const provider = {
      getVehiclesForLine: jest.fn(
        () =>
          new Promise<(typeof vehicle)[]>((resolve) => {
            complete = resolve;
          }),
      ),
    };
